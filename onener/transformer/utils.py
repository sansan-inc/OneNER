...
@dataclass
class InputFeaturesCustom(InputFeatures):
    """
    A single set of features of data.
    Property names are the same names as the corresponding inputs to a model.
    Add valid_mask for arguments for custom models such as crf.
    """
...
class TokenClassificationTask:
    ...
    def convert_examples_to_features(
        ...
        return_custom: bool = False
    ) -> Union[List[InputFeatures], List[InputFeaturesCustom]]:
        """Loads a data file into a list of `InputFeatures`
        `cls_token_at_end` define the location of the CLS token:
            - False (Default, BERT/XLM pattern): [CLS] + A + [SEP] + B + [SEP]
            - True (XLNet/GPT pattern): A + [SEP] + B + [SEP] + [CLS]
        `cls_token_segment_id` define the segment id associated to the CLS token (0 for BERT, 2 for XLNet)
        `return_custom` defines the use of a custom model:
            - False: Use AutoModelForTokenClassification
            - True: Use AutoModelForCrfNer etc...
        """
